 ...
 *
 * Shared between the main process (TelnetSessionService tokenizes each line as it
 * arrives) and the renderer (the AI chat echoes a terminal selection with the same
 * highlighting). It must stay free of DOM or Node dependencies.
 */

import type { TerminalLineChunk, TerminalOverlaySpan, TerminalTokenKind, TerminalTokenSpan } from './terminal'
import { findMatchingBracket, keepOutermostSpans } from './jsonUtils'
import { JSON_INDENT_WIDTH } from './jsonIndent'
...
// BrightScript operators and structural punctuation. Colored only within detected
// code regions (see computeCodeHighlightRanges) so log prose is left untouched.
const OPERATOR_RE = /[-+*\/=<>.(),]/g

const JSON_TOKEN_RE = /"(?:[^"\\]|\\.)*"|-?\d+(?:\.\d+)?(?:e[+-]?\d+)?|\btrue\b|\bfalse\b|\bnull\b|[{}\[\]:,]/g

// Above this length, skip JSON overlay detection for the line. detectAndTokenizeJson probes every
// bracket, so a huge single-line blob (especially non-JSON brace soup) is superlinear. A line this
// long loses only the expand affordance, and the tokenizer runs per line in the main process, so an
// unbounded scan here would stall the debug stream. Real single-line JSON payloads are far shorter.
...
// Operators sit just below keywords: they only fill punctuation gaps in code and
// must never override a keyword, string, number, or any other code token.
const PRIORITY_OPERATOR = 199
...
/**
 * Determines which character ranges within a terminal line are BrightScript *code*
 * and therefore eligible for keyword and operator highlighting. Only the debugger's
 * own code is treated as code:
 *  - Source-line rows (NNN: / NNN:*): everything after the line-number prefix.
 *  - The debugger prompt echo: the statement typed after "BrightScript Debugger>".
 *  - Debugger command-list rows: the command column only (not the prose description).
 *
 * Every other line is log / prose output, so it gets no keyword highlighting. This
 * is deliberate: applying keyword patterns line-wide colored plain English words that
 * happen to be keywords (e.g. "option" in a log line).
 *
 * @param text - The raw terminal line text.
 * @returns Array of character ranges within which code-token patterns should apply.
 */
function computeCodeHighlightRanges(text: string): Array<{ start: number; end: number }> {
    if (!text) return []
    if (DEBUGGER_HELP_PROSE_RE.test(text)) return []

    // Source-line rows: the source follows the "NNN:" (+ optional "*") prefix.
    const sourceLine = text.match(SOURCE_LINE_RE)
    if (sourceLine && sourceLine.index !== undefined) {
        const codeStart = sourceLine.index + sourceLine[0].length
        return codeStart < text.length ? [{ start: codeStart, end: text.length }] : []
    }

    // Debugger prompt echo: the user-typed statement follows the prompt.
    const prompt = text.match(BRIGHTSCRIPT_DEBUGGER_PROMPT_RE)
    if (prompt) {
        const codeStart = prompt[0].length
        return codeStart < text.length ? [{ start: codeStart, end: text.length }] : []
    }

    // Debugger command-list rows: highlight only the command column, not the prose.
...
function detectAndTokenizeJson(text: string): { overlays: TerminalOverlaySpan[]; spans: TerminalTokenSpan[]; seenKeys: Set<string> } {
    if (text.length > MAX_JSON_SCAN_CHARS) return { overlays: [], spans: [], seenKeys: new Set() }
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        try {
            ...
            candidates.push({
                ...
                rawForTokenize: raw,
                pretty: JSON.stringify(parsed, null, JSON_INDENT_WIDTH)
            })
            // Skip past this whole match: any brackets inside it are nested candidates that
            // keepOutermostSpans would discard anyway, so probing them is wasted work.
            // This is what keeps a large valid JSON object linear instead of quadratic.
            i = end
        } catch {
            // Not strict JSON
        }
    ...
    while (true) {
        ...
        try {
            ...
            candidates.push({
                ...
                rawForTokenize: null,
                pretty: JSON.stringify(parsedInner, null, JSON_INDENT_WIDTH)
            })
    ...
    const spans: TerminalTokenSpan[] = []
    const filtered = keepOutermostSpans(candidates)
...
export function tokenizeTerminalLine(text: string, blockSeverity?: 'warning' | 'error'): TerminalLineChunk {
    ...
    const codeRanges = computeCodeHighlightRanges(text)
    ...
    applyRegexInRanges(BRIGHTSCRIPT_KEYWORD_RE, 'keyword', PRIORITY_KEYWORD, codeRanges)
    applyRegexInRanges(VB_KEYWORD_RE, 'keyword', PRIORITY_KEYWORD, codeRanges)
    applyRegexInRanges(OPERATOR_RE, 'objectPunctuation', PRIORITY_OPERATOR, codeRanges)
    ...
    const debuggerPrompt = text.match(BRIGHTSCRIPT_DEBUGGER_PROMPT_RE)
    if (debuggerPrompt) {
        // Wash only the prompt itself. The statement typed after it is BrightScript
        // code (see computeCodeHighlightRanges) and is highlighted as such.
        apply(0, debuggerPrompt[0].length, 'brightscriptDebuggerPrompt', PRIORITY_DEBUGGER_PROMPT)
    }
    ...
    // String spans, so the log-tag / level coloring below does not override text
    // inside a string literal (e.g. "[App] ..." must stay a string, not a log tag).
    const stringRanges: Array<{ start: number; end: number }> = []
    STRING_RE.lastIndex = 0
    for (let stringMatch = STRING_RE.exec(text); stringMatch; stringMatch = STRING_RE.exec(text)) {
        if (!stringMatch[0]) { STRING_RE.lastIndex += 1; continue }
        stringRanges.push({ start: stringMatch.index, end: stringMatch.index + stringMatch[0].length })
    }
    const insideString = (start: number, end: number): boolean =>
        stringRanges.some(range => overlaps(start, end, range.start, range.end))
    ...
    while (true) {
        ...
        if (!tag) continue
        if (insideString(match.index, match.index + match[0].length)) continue
