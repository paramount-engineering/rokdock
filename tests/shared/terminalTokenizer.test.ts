...
describe('tokenizeTerminalLine - JSON overlay', () => {
    it('produces a json overlay for an inline JSON object', () => {
        ...
        // value is pretty-printed with the shared JSON_INDENT_WIDTH (2 spaces), so a
        // payload opened from the terminal matches the JSON editor's own formatting.
        const parsed = JSON.parse(jsonOverlay!.value)
        expect(parsed).toEqual({ key: 'value' })
        expect(jsonOverlay!.value).toBe('{\n  "key": "value"\n}')
    })
    ...
    it('keeps only the outermost overlay for nested JSON', () => {
        const text = '{"a":{"b":1}}'
        const jsonOverlays = tokenizeTerminalLine(text).overlays.filter(overlay => overlay.kind === 'json')
        expect(jsonOverlays).toHaveLength(1)
        expect(jsonOverlays[0]!.start).toBe(0)
        expect(jsonOverlays[0]!.end).toBe(text.length)
    })

    it('produces a separate overlay for each sibling JSON object on a line', () => {
        const jsonOverlays = tokenizeTerminalLine('{"a":1} and {"b":2}').overlays.filter(overlay => overlay.kind === 'json')
        expect(jsonOverlays).toHaveLength(2)
    })

    it('skips JSON detection on a line past the scan cap so a huge blob cannot stall the tokenizer', () => {
        // Valid JSON, but longer than MAX_JSON_SCAN_CHARS (20000): detection is skipped, so no overlay.
        const huge = '{"x":"' + 'a'.repeat(20001) + '"}'
        expect(huge.length).toBeGreaterThan(20000)
        const result = tokenizeTerminalLine(huge)
        expect(result.overlays.find(overlay => overlay.kind === 'json')).toBeUndefined()
    })
})
...
describe('tokenizeTerminalLine - keyword highlighting', () => {
    it('highlights a BrightScript keyword inside a source-line row', () => {
        // A "NNN:" prefix marks the rest of the line as BrightScript code.
        const text = '042: if x = 1 then return true'
        const ifStart = text.indexOf('if')
        expect(kindAt(text, ifStart)).toBe('keyword')
    })

    it('does NOT highlight keywords in a plain log line', () => {
        // A log line is not code, so a plain word that happens to be a VB/BrightScript
        // keyword (here "option") must stay plain rather than being keyword-colored.
        const text = '[ContentService] bitrate option: hd  4800kbps'
        const optionStart = text.indexOf('option')
        expect(kindAt(text, optionStart)).toBe('plain')
    })

    it('highlights the statement typed after the debugger prompt', () => {
        // The prompt itself is washed. The echoed statement after it is code.
        const text = 'BrightScript Debugger> if true then'
        const result = tokenizeTerminalLine(text)
        expect(result.tokens[0]!.kind).toBe('brightscriptDebuggerPrompt')
        expect(kindAt(text, text.indexOf('if'))).toBe('keyword')
        expect(kindAt(text, text.indexOf('true'))).toBe('boolean')
    })

    it('colors operators inside a code region as objectPunctuation', () => {
        const text = '042: m.content = data'
        expect(kindAt(text, text.indexOf('.'))).toBe('objectPunctuation')
        expect(kindAt(text, text.indexOf('='))).toBe('objectPunctuation')
    })

    it('keeps a bracketed tag inside a string literal as string, not a log tag', () => {
        // The "[App]" is inside the print string, so it must stay string-colored
        // rather than being hijacked by the log-tag regex.
        const text = '056: print "[App] diagnostics report"'
        expect(kindAt(text, text.indexOf('[App]'))).toBe('string')
    })
