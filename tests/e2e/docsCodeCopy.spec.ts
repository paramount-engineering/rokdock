/**
 * E2E: the hover "Copy" button on fenced code blocks in the Developer Docs
 * reading pane. Uses the offline disk cache to render a real page (with a code
 * block) deterministically, with no network.
 */
...
function seedPageWithCode(userData: string): string {
    ...
    const markdown = `# Roku Developer Program\n\nA snippet:\n\n\`\`\`brightscript\n${CODE}\n\`\`\`\n`
    const page = { path: pagePath, title: 'Roku Developer Program', markdown }
...
test('code block: hover copy button copies the snippet and shows a copied state', async () => {
    const userData = fs.mkdtempSync(path.join(os.tmpdir(), 'rokdock-codecopy-'))
    const pageTitle = seedPageWithCode(userData)
    ...
    try {
        ...
        const copyBtn = win.locator('.docs-code-copy')
        await expect(copyBtn).toHaveCount(1, { timeout: 6000 })
        await copyBtn.click()

        // The button flips to its "done" state on a successful copy.
        await expect(copyBtn).toHaveClass(/docs-code-copy--done/, { timeout: 3000 })

        // The clipboard holds the snippet, cleanly (no trailing per-line
        // whitespace, matching what the highlighter renders).
        const clip = await win.evaluate(() => navigator.clipboard.readText())
        expect(clip).toContain('sub Main()')
        expect(clip).toContain('print "hello world"')
        expect(clip).toContain('end sub')
